const API_KEY = "e8938208c20a15b1138487810fe0e353";
const weather = document.querySelector(".js-weather");
const COORDS = `coords`;
/**api => 다른 서버로부터 쉽게 데이터를 가져올 수 있는 수단. 컴퓨터끼리 소통하기 위해 고안됨.*/

function getWeather(lat, lng) {
  /**fetch=>api로부터 데이터를 받아오는 메소드. */
  fetch(
    `http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lng}&appid=${API_KEY}&units=metric`
  )
    .then(function (response) {
      //response 인자 안에 api fetch가 자동으로 return한 날씨 정보의 response stream이 담김.
...
function coordsToStorage(userLoaction) {
  localStorage.setItem(
    COORDS,
    JSON.stringify(userLoaction)
...
function handleGeoSuccess(position) {
  const lat =
    position.coords
      .latitude; /**chrome 콘솔에서 console.dir해서 찾아볼 수 있음. */
  const lng = position.coords.longitude;
  const userLoaction = {
    lat,
    lng /**(객체 key)lat : (위에 선언된 const)lat을 축약하여 표현한 것. lng도 마찬가지. */,
  };
  coordsToStorage(userLoaction);
